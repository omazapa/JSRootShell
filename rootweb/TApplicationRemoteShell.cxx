...
//////////////////////////////////////////////////////////////////////////
//                                                                      //
// TApplicationRemoteShell                                                   //
//                                                                      //
// TApplicationRemoteShell maps a remote session. It starts a remote session //
...
//
// TApplicationRemoteShell Interrupt signal handler
//______________________________________________________________________________
Bool_t TARInterruptHandler::Notify()
{
   // TApplicationRemoteShell interrupt handler.

   Info("Notify","Processing interrupt signal ...");

   // Handle interrupt condition on socket(s)
   fApplicationRemoteShell->Interrupt(kRRI_Hard);

   return kTRUE;
}


// ClassImp(TApplicationRemoteShell)

static const char *gScript = "roots";
static const char *gScriptCmd = "\\\"%s %d localhost:%d/%s -d=%d\\\"";
#ifndef WIN32
static const char *gSshCmd = "sshpass -p '%s' ssh  -o StrictHostKeyChecking=no %s -f4 %s -R %d:localhost:%d sh -c \
   \"'(sh=\\`basename \'\\\\\\$SHELL\'\\`; \
...
Int_t TApplicationRemoteShell::fgPortAttempts = 100; // number of attempts to find a port
Int_t TApplicationRemoteShell::fgPortLower =  49152; // lower bound for ports
Int_t TApplicationRemoteShell::fgPortUpper =  65535; // upper bound for ports

//______________________________________________________________________________
TApplicationRemoteShell::TApplicationRemoteShell(const char *url,const char *pass, Int_t debug,
                                       const char *script)
                   : TApplication(), fUrl(url)
{
   ...
   if (!ss || !ss->IsValid()) {
      Error("TApplicationRemoteShell","unable to find a free port for connections");
   ...
   if (script && *script) {
      ...
      if (script[1] == '<') {
         if (script[2])
            ...
            Error("TApplicationRemoteShell", "illegal script name <");
      } else
         sc = script;
   }
   sc.ReplaceAll("\"","");
   ...
   TString cmd;
   cmd.Form(gSshCmd,pass, verb, userhost.Data(), rport, port, scriptCmd.Data(), scriptCmd.Data());
#ifdef WIN32
...
#endif
   if (gDebug > 0)
      Info("TApplicationRemoteShell", "executing: %s", cmd.Data());
   if (gSystem->Exec(cmd) != 0) {
      Info("TApplicationRemoteShell", "an error occured during SSH connection");
      mon->DeActivateAll();
      delete mon;
      delete ss;
      SafeDelete(fSocket);
   ...
   if (!(fSocket = ss->Accept())) {
      Error("TApplicationRemoteShell", "failed to open connection");
      SetBit(kInvalidObject);
      return;
   }
   ...
   if (fSocket->Recv(buf, sizeof(buf), what) <= 0) {
      Error("TApplicationRemoteShell", "failed to receive startup message");
      SafeDelete(fSocket);
      SetBit(kInvalidObject);
      return;
   }
   ...
   if (fSocket->Recv(fProtocol, what) != 2*sizeof(Int_t)) {
      Error("TApplicationRemoteShell", "failed to receive remote server protocol");
      SafeDelete(fSocket);
      SetBit(kInvalidObject);
      return;
   }
   if (fProtocol != kRRemote_Protocol)
      Info("TApplicationRemoteShell","server runs a different protocol version: %d (vs %d)",
                     fProtocol, kRRemote_Protocol);

   TMessage *msg = 0;
   // Receive the protocol version run remotely
   if (fSocket->Recv(msg) < 0 || msg->What() != kMESS_ANY) {
      Error("TApplicationRemoteShell", "failed to receive server info - protocol error");
      SafeDelete(fSocket);
      SetBit(kInvalidObject);
      return;
   }
...
//______________________________________________________________________________
TApplicationRemoteShell::~TApplicationRemoteShell()
{
   // Destructor
...
//______________________________________________________________________________
Int_t TApplicationRemoteShell::Broadcast(const TMessage &mess)
{
...
//______________________________________________________________________________
Int_t TApplicationRemoteShell::Broadcast(const char *str, Int_t kind, Int_t type)
{
...
//______________________________________________________________________________
Int_t TApplicationRemoteShell::BroadcastObject(const TObject *obj, Int_t kind)
{
...
//______________________________________________________________________________
Int_t TApplicationRemoteShell::BroadcastRaw(const void *buffer, Int_t length)
{
...
//______________________________________________________________________________
Int_t TApplicationRemoteShell::Collect(Long_t timeout)
{
...
//______________________________________________________________________________
Int_t TApplicationRemoteShell::CollectInput()
{
...
//______________________________________________________________________________
void TApplicationRemoteShell::RecvLogFile(Int_t size)
{
...
//______________________________________________________________________________
Int_t TApplicationRemoteShell::SendObject(const TObject *obj)
{
...
//______________________________________________________________________________
Bool_t TApplicationRemoteShell::CheckFile(const char *file, Long_t modtime)
{
...
//______________________________________________________________________________
Int_t TApplicationRemoteShell::SendFile(const char *file, Int_t opt, const char *rfile)
{
...
//______________________________________________________________________________
void TApplicationRemoteShell::Terminate(Int_t status)
{
...
//______________________________________________________________________________
void TApplicationRemoteShell::SetPortParam(Int_t lower, Int_t upper, Int_t attempts)
{
   ...
   ::Info("TApplicationRemoteShell::SetPortParam","port scan: %d attempts in [%d,%d]",
          fgPortAttempts, fgPortLower, fgPortUpper);
}

//______________________________________________________________________________
Long_t TApplicationRemoteShell::ProcessLine(const char *line, Bool_t, Int_t *)
{
...
//______________________________________________________________________________
void TApplicationRemoteShell::Print(Option_t *opt) const
{
   // Print some info about this instance

   TString s(Form("OBJ: TApplicationRemoteShell     %s", fName.Data()));
   Printf("%s", s.Data());
   if (opt && opt[0] == 'F') {
      s = "    url: ";
      if (strlen(fUrl.GetUser()) > 0)
...
//______________________________________________________________________________
void TApplicationRemoteShell::Interrupt(Int_t type)
{
...
//______________________________________________________________________________
void TApplicationRemoteShell::Browse(TBrowser *b)
{
